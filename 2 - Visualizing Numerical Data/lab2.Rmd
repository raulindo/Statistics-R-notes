...
# Cria dataframe com dados de vôos
df = nycflights
# Separa com destino apenas para SFO e Mês de Fevereiro (2)
df_SFO_feb = df[(df$dest=='SFO' & df$month==2),]

#To get number of rows use function NROW
NROW(df_SFO_feb)
...
# Histogram plot
ggplot(data = df_SFO_feb, aes(x = arr_delay)) +
  geom_histogram(binwidth = 15)

# Summary
rdu_flights %>%
  summarise(mean_dd = mean(dep_delay), sd_ad = sd(arr_delay), n = n())
```
...
# Mutate and create variabel called avg_speed
nycflights <- nycflights %>%
  mutate(avg_speed = distance/air_time) 

# Show only 10 first rows of ordered selection 
head(arrange(select(nycflights, avg_speed, tailnum), desc(avg_speed)), 10)
...
ggplot(nycflights, aes(y = avg_speed, x = distance)) + geom_point()
...
```{r on-time-arr-perc}
# Create variabel based on arrival delay: if less or equal zero, arrive is on-time, else is delayed
nycflights <- nycflights %>%
  mutate(arr_type = ifelse(arr_delay <= 0, "on time", "delayed"))

# Group by dep_type and calculate the on-time arrival rate for arr_type == 'on-time'
nycflights %>%
  group_by(dep_type) %>%
